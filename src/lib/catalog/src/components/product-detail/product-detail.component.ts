  ...
  implements OnInit, OnDestroy
{
  ...
  detailForm = this.formBuilder.group({
    ...
    customFields: this.formBuilder.group(
      getCustomFieldsDefaults(this.customFields)
    ),
  });
  ...
  ngOnInit() {
    ...
    const productFacetValues$ = this.isNew$.pipe(
      switchMap((isNew) => {
        return isNew
          ? of([])
          : this.entity$.pipe(map((product) => product.facetValues));
      })
    );
    const productGroup = this.detailForm;
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
    const formFacetValueIdChanges$ = productGroup
      .get("facetValueIds")!
      .valueChanges.pipe(
        distinctUntilChanged(),
        switchMap((ids) =>
      ...
      .subscribe(
        ...
        (err) => {
          this.notificationService.error(
            _("catalog.notify-remove-product-from-channel-error")
          );
        }
      );
  }
  ...
  removeVariantFromChannel({
    ...
    from(getChannelCodeFromUserStatus(this.dataService, channelId))
      ...
      .subscribe(
        () => {
          this.notificationService.success(
            _("catalog.notify-remove-variant-from-channel-success")
          );
        },
        (err) => {
          this.notificationService.error(
            _("catalog.notify-remove-variant-from-channel-error")
          );
        }
      );
  }
  ...
  updateSlug(nameValue: string) {
    ...
    const currentTranslation = this.entity
      ? findTranslation(this.entity, this.languageCode)
      : undefined;
      ...
      .pipe(
        ...
        mergeMap(([product, languageCode]) => {
          ...
          return this.productDetailService.updateProduct({
            ...
            languageCode,
            autoUpdate:
              this.detailForm.get(["autoUpdateVariantNames"])?.value ?? false,
            productInput,
            variantsInput,
          });
        })
  ...
  private updateSlugAfterSave(
    results: Array<UpdateProductMutation | UpdateProductVariantsMutation>
  ) {
