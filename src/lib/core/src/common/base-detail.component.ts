...
> implements DeactivateAware
{
  ...
  init() {
    this.entity$ = this.route.data.pipe(
      switchMap((data) =>
        (data.entity as Observable<Entity>).pipe(takeUntil(this.destroy$))
      ),
  ...
  protected abstract setFormValues(
    entity: Entity,
    languageCode: LanguageCode
  ): void;
...
> extends BaseDetailComponent<NonNullable<ResultOf<T>[Field]>> {
  ...
  override init() {
    this.entity$ = this.route.data.pipe(
      switchMap((data) =>
        (data.detail.entity as Observable<ResultOf<T>[Field]>).pipe(
...
>(config: {
  ...
  return {
    ...
    breadcrumbFn: (result: any) =>
      config.getBreadcrumbs?.(result) ?? ([] as BreadcrumbValue[]),
